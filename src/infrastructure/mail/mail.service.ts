...
export class MailService {
  ...
  private get fromAddress() {
    return `"FieldOps" <${this.config.get<string>('MAIL_USER')}>`;
  }
  ...
  async sendBookingConfirmation(to: string, data: BookingConfirmationData): Promise<void> {
    try {
      await this.transporter.sendMail({
        ...
        to,
        subject: '✅ Xác nhận đặt sân thành công - FieldOps',
  ...
  async sendBookingCancelled(to: string, data: BookingCancelledData): Promise<void> {
    try {
      await this.transporter.sendMail({
        from: this.fromAddress,
        to,
        subject: '❌ Đặt sân đã bị hủy - FieldOps',
        html: this.bookingCancelledHtml(data),
      });
  ...
  async sendWelcome(to: string, name: string): Promise<void> {
    try {
      await this.transporter.sendMail({
        from: this.fromAddress,
        to,
        subject: '👋 Chào mừng bạn đến với Booking Field!',
  ...
  async sendPaymentConfirmation(
    to: string,
    data: { name: string; amount: number; bookingId: string },
  ): Promise<void> {
    try {
      await this.transporter.sendMail({
        from: this.fromAddress,
        to,
        subject: '💳 Thanh toán thành công - FieldOps',
        html: this.paymentConfirmationHtml(data),
      });
    } catch (err) {
  ...
  private bookingConfirmationHtml(data: BookingConfirmationData): string {
    return `
<!DOCTYPE html>
<html>
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
      ...
      <div style="background:#f9fafb;border-radius:8px;padding:20px;margin:20px 0;">
        <table style="width:100%;border-collapse:collapse;">
          ...
          <tr style="border-top:1px solid #e5e7eb;">
            ...
            <td style="padding:10px 0;color:#16a34a;font-weight:700;text-align:right;font-size:16px;">${data.amount.toLocaleString('vi-VN')} VNĐ</td>
          </tr>
        </table>
      </div>
    ...
    <div style="background:#f9fafb;padding:20px;text-align:center;">
      <p style="color:#9ca3af;font-size:12px;margin:0;">FieldOps - Hệ thống quản lý sân thể thao</p>
    </div>
  </div>
</body>
</html>`;
  ...
  private bookingCancelledHtml(data: BookingCancelledData): string {
    return `
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"></head>
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
    ...
    <div style="padding:32px;">
      ...
      <p style="color:#374151;">Tài khoản của bạn đã được tạo thành công. Bạn có thể bắt đầu đặt sân ngay bây giờ!</p>
    </div>
    <div style="background:#f9fafb;padding:20px;text-align:center;">
      <p style="color:#9ca3af;font-size:12px;margin:0;">FieldOps - Hệ thống quản lý sân thể thao</p>
    </div>
  ...
  private paymentConfirmationHtml(data: {
    name: string;
    amount: number;
    bookingId: string;
  }): string {
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
    ...
    <div style="padding:32px;">
      <p style="color:#374151;font-size:16px;">Xin chào <strong>${data.name}</strong>,</p>
      <p style="color:#374151;">Chúng tôi đã nhận được thanh toán của bạn.</p>
      <div style="background:#f9fafb;border-radius:8px;padding:20px;margin:20px 0;">
